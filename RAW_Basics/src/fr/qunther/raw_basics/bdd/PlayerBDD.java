package fr.qunther.raw_basics.bdd;
...
public class PlayerBDD {
	...
	public static boolean isPlayerInBDDbyNickName(String nickname) {
		ArrayList<Object> arguments = new ArrayList<Object>();
	...
	public static String getUUIDfromNickname(String nickname) {
		ArrayList<Object> arguments = new ArrayList<Object>();
		arguments.add(nickname);
		
		String sql = "SELECT uuid FROM player WHERE nickname=?";
		return (String) QuerySender.sendGetterQuery(sql, arguments).get(0);
	}
...
SELECT ROW_NUMBER() OVER () AS player_ranking FROM ( SELECT uuid
   ,  count(case when classement = 1 then 1 else NULL end) as premier_count
   ,  sum(kills) as kills_sum
 FROM  a_joue_partie
 GROUP BY uuid ORDER by premier_count DESC) a;
	...
	SELECT uuid
	  ...
	  ,  sum(kills) as kills_sum
	FROM  a_joue_partie
	GROUP BY uuid ORDER by premier_count DESC, kills_sum DESC;
	
	ROW_NUMBER() OVER () AS player_ranking
	
*/	
	
	public static ArrayList<Object> getPlayerChaosClassementInfos(String uuid) {
		ArrayList<Object> arguments = new ArrayList<Object>();
		arguments.add(uuid);
		...
		String sql = "	SELECT uuid"
				+ "	   ,  count(case when classement = 1 then 1 else NULL end) as premier_count"
				+ "	   ,  sum(kills) as	 kills_sum"
				+ "	 FROM  a_joue_partie WHERE uuid=?"
				+ "	 GROUP BY uuid ORDER by premier_count DESC, kills_sum DESC;";
		return (ArrayList<Object>) QuerySender.sendGetterQuery(sql, arguments);
	}
	
	public static ArrayList<Object> getChaosClassementTop(int top) {
		ArrayList<Object> arguments = new ArrayList<Object>();
		arguments.add(top);
		...
		String sql = "SELECT uuid"
				...
				+ "   ,  sum(kills) as kills_sum"
				+ " FROM  a_joue_partie"
				+ " GROUP BY uuid ORDER by premier_count DESC, kills_sum DESC LIMIT 1 OFFSET ?-1";
		return (ArrayList<Object>) QuerySender.sendGetterQuery(sql, arguments);
	}

	
	@SuppressWarnings("unchecked")
	public static ArrayList<Object> getPlayerChaosOverallKills(String uuid) {
		ArrayList<Object> arguments = new ArrayList<Object>();
		arguments.add(uuid);
		
		String sql = "select uuid,sum(kills) from a_joue_partie group by uuid having uuid=?";
		return (ArrayList<Object>) QuerySender.sendGetterQuery(sql, arguments).get(1);
	}
	
	public static ArrayList<Object> getPlayerChaosOverallWins(String uuid) {
		ArrayList<Object> arguments = new ArrayList<Object>();
		arguments.add(uuid);
